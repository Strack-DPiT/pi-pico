...
class EPD_2in13_V4_Portrait(framebuf.FrameBuffer):
    def _init_(self):
        ...
        super()._init_(self.buffer, self.width, self.height, framebuf.MONO_HLSB)
        self.init()
...
class EPD_2in13_V4_Landscape(framebuf.FrameBuffer):
    def _init_(self):
        ...
        self.buffer = bytearray(self.height * self.width // 8)
        super()._init_(self.buffer, self.height, self.width, framebuf.MONO_VLSB)
        self.init()
...
if name=='__main':
    ...
    epd.fill_rect(110, 60, 60, 20, 0xff)
    epd.text(str(lat), 110, 61, 0x00)
    epd.text(str(long),110,71,0x00)
    epd.displayPartial(epd.buffer)
    epd.delay_ms(2000)
